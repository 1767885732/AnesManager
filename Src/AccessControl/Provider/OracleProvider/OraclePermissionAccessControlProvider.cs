...
{
    public partial class OracleAccessControlProvider
    {
        ...
        const string GET_ALL_PERMISSION_BY_APP_FLAG = "SELECT T1.PERMISSION_ID, T1.PERMISSION_NAME, T1.RESOURCE_ID, T1.OPERATION, T1.ALLOWED, T1.EXTEND_INFO, T2.APP_FLAG FROM MED_IMEDICAL_PERMISSIONS T1 JOIN MED_IMEDICAL_RESOURCES T2 ON T1.RESOURCE_ID = T2.RESOURCE_ID WHERE T2.APP_FLAG = :APP_FLAG";
...
JOIN MED_IMEDICAL_ROLES_PERMISSIONS t2 ON t2.PERMISSION_ID = t1.permission_id
JOIN MED_IMEDICAL_USERS_ROLES t3 ON t2.role_id = t3.role_id
JOIN MED_IMEDICAL_RESOURCES t4 ON t1.RESOURCE_ID = t4.RESOURCE_ID
WHERE t3.user_id = :USER_ID AND t4.APP_FLAG = :APP_FLAG";
