...
namespace Wis.Anes
{
    ...
    /// 基类窗口代码模块
    /// </summary>
    public partial class ShellFrm : Form
    {
        ...
        private Font _titleFont = new Font("微软雅黑", 13.2f);
        ...
        private void PaintTitlePanel()
        {
            using (Graphics g = pnlTop.CreateGraphics())
            {
                ...
                if (!string.IsNullOrEmpty(MedConfiguration.HospitalName))
                {
                    g.DrawString(Text + "（" + MedConfiguration.HospitalName + "）", _titleFont, new SolidBrush(_titleFontColor), 20, (pnlTop.Height - g.MeasureString("A", _titleFont).Height) / 2 + 2);
                }
                else
                {
                    g.DrawString(Text, _titleFont, new SolidBrush(_titleFontColor), 20, (pnlTop.Height - g.MeasureString("A", _titleFont).Height) / 2 + 2);
                }
                #endregion 画窗体图标
            }
        }
        ...
        private void ShellFrm_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                this.topBarControl1.Height = 100;
                //LoadMask.Show();
                RefreshMenuContainer();
                AnesMenuCollection.NavBarContainer = horizontalNavBar1;
                label3.Text ="您好:"+ ExtendApplicationContext.Current.LoginUserContext.UserName;
                MainForm_Load();
            }
            catch (Exception ex)
            {
                ExceptionHandler.Handle(ex);
                Sundries.MessageBox("程序启动出现异常，请查看日志文件！", MessageBoxIcon.Error);
                Application.Exit();
            }
            finally
            {
                //LoadMask.Hide();
                //IsCurrentPatInRescue();
            }
        }

        //刷新菜单容器
        private void RefreshMenuContainer()
        {
            ClearMenu(flowLayoutPanel1, horizontalNavBar1);
            horizontalNavBar1.Width = flowLayoutPanel2.Width;
            ApplicationManager.Init(AnesMenuCollection.CurrentMenuCollection, workSpaceControl1, flowLayoutPanel1, horizontalNavBar1);
            ApplicationManager.CurrentMenuActivated += ApplicationManager_CurrentMenuActivated;
            if (flowLayoutPanel1.Controls != null && flowLayoutPanel1.Controls.Count > 0 && flowLayoutPanel1.Controls[0] is IcuMenuButton)
            {
                (flowLayoutPanel1.Controls[0] as IcuMenuButton).PerformClick();
            }
        }

        private void ApplicationManager_CurrentMenuActivated(ApplicationFramework.Model.Menu obj)
        {
            string menuCaption = obj.MenuName;
            ...
            {
                if (obj.Parent.MenuName.Contains("大事件") && (!Wis.Anes.Framework.AccessControl.CheckBrowseRight("大事件") || !Framework.AccessControl.CheckModifyRightForOperator("麻醉单"))) return;
            }
            else if (ExtendApplicationContext.Current.AppType == ApplicationType.PACU)
            {
                if (obj.Parent.MenuName.Contains("大事件") && (!Framework.AccessControl.CheckBrowseRight("大事件"))) return;
            }

            //if ((obj.Parent.SubMenus[0] != obj) && !Framework.AccessControl.CheckBrowseRight(menuCaption))            
            //{
            //    MessageBox.Show("无权限");
            //    return;
            //}
            if (menuCaption.Equals(ViewNames.LockPatient) && ExtendApplicationContext.Current.OperationStatus.Equals(OperationStatus.Done))
            {
                return;
            }

            if (obj.Parent.MenuName.Contains("大事件"))
            {
                leftBarControl1.RaiseEvent("飘窗@" + menuCaption);
            }
            else
            {
                leftBarControl1.RaiseEvent(menuCaption);
            }
        }
        #endregion 窗体事件
        ...
        private void InitalizeUI()
        {
            ...
            {
                // PopulateCodeTables();
                this.topBarControl1.Width = flowLayoutPanel2.Width;
                this.topBarControl1.MecicalDocBarControl.Width = flowLayoutPanel2.Width;

                //ExtendApplicationContext.Current.ConfigTable = ConfigurationProxy.GetConfigTableDataTable();
                lblUserName.Text = "当前用户：" + ExtendApplicationContext.Current.LoginUserContext.UserName;
        ...
        {
            CommonProxy.ExecuteNonQuery(string.Format("UPDATE WIS_OPER_MASTER SET OPER_STATUS = 80 WHERE (OUT_DATE_TIME + {0}) <= GETDATE()", ApplicationConfiguration.OperationDoneDays));
        }
        ...
        private void PopulateCodeTables()
        {
            ...
            foreach (KeyValue keyValue in list)
            {
                if (!ExtendApplicationContext.Current.CodeTables.ContainsKey(keyValue.Value))
                {
                    ...
                    else if (keyValue.Value.ToUpper() == "WIS_DICT_ANES_INPUT_DOCTOR".ToUpper())//录入字典表
                    {
        ...
        private void ChangePatient()
        {
            if (!ApplicationConfiguration.IsPACUProgram)
            {
                int flag = IsMatchingUser();//2可以编辑//1提示可以编辑//0提示不可以编辑
                if (flag != 2)
                {
                    if (flag == 0)
                    {
                        //topBarControl1.Enabled = false;
                        MessageBox.Show("该病人麻醉医生不是登录用户");
                    }
                    if (flag == 1)
                    {
                        //topBarControl1.Enabled = true;
                        MessageBox.Show("该病人麻醉医生不是登录用户");
                    }
                    ExtendApplicationContext.Current.IsMatchingUser = flag;
                }
                else
                {
                    topBarControl1.Enabled = true;
            ...
            foreach (Control ctl in topBarControl1.MecicalDocBarControl.Controls)
            {
                if (ctl is FlowLayoutPanel)
                {
                    foreach (Control ctlItem in ctl.Controls)
                        if (ctlItem is SimpleButton)
                            ctlItem.Click += new EventHandler(docButton_Click);
                }
            }
        ...
        private void leftBarControl1_ViewChangedEventHandler(object sender, ViewChangedEventArgs e)
        {
            if (e.ViewName.StartsWith("飘窗@"))  //飘窗
            {
                ...
                if (!string.IsNullOrEmpty(key))
                {
                    ...
                    if (!(FloatFrm.isFloat))
                    {
                        ...
                        floatFrm.StartPosition = FormStartPosition.CenterScreen;
                        floatFrm.ShowDialog();
            ...
            else
            {
                    ...
                    else
                    {
                        try
                        {
                            if (!string.IsNullOrEmpty(ExtendApplicationContext.Current.PatientInformation.PatientID))
                            {
                                CommonProxy.ExecuteNonQuery(string.Format("UPDATE WIS_OPER_MASTER SET OPER_STATUS = 80 WHERE (OUT_DATE_TIME + {0}) <= GETDATE()", ApplicationConfiguration.OperationDoneDays));
                                //龙山病历接口20211203
                                string webPath = ConfigurationHelper.Read("EMRWebPath");
                                if (!string.IsNullOrEmpty(webPath))
                                {
                                    string patid = ExtendApplicationContext.Current.PatientContext.PatientID;
                                    string visitid = ExtendApplicationContext.Current.PatientContext.VisitID.ToString();
                                    string username = "ys01";
                                    DataTable dtable = CommonProxy.GetDataFromSQLString("select DEPT_STAYED from WIS_OPER_MASTER where PAT_ID='" + patid + "' and VISIT_ID='" + visitid + "' ");
                                    string dept = "";
                                    if (dtable.Rows.Count > 0)
                                    {
                                        dept = dtable.Rows[0]["DEPT_STAYED"].ToString();
                                    }
                                    string epid = "";

                                    DictDA _dictDA = new DictDA();
                                    Dict.WIS_HIS_PATDataTable _td = _dictDA.GetWisDisPat(patid, visitid);
                                    if (_td.Rows.Count > 0)
                                    {
                                        epid = _td.Rows[0]["HIS_VISIT_ID"].ToString();
                                    }
                                    else
                                    {
                                        Dialog.MessageBox("提示：未查到病人的就诊号码。");
                                        return;
                                    }

                                    string path = "TPSID=10009334&USERNAME=ys01&DEPARTMENT=" + dept + "&EpisodeID=" + epid;



                                    webPath += path;
                                    Process.Start(webPath);
                                    //add by dhc 20200526 web访问电子病历
                                    //string token = "";
                                    //token += ConfigurationHelper.Read("EMRAccount") + "|";
                                    //string passBase64 = ConfigurationHelper.Read("EMRPassword");
                                    //byte[] passByte = Encoding.Default.GetBytes(passBase64);
                                    //passBase64 = Convert.ToBase64String(passByte);
                                    //token += passBase64 + "|";
                                    //token += ConfigurationHelper.Read("PortalCode") + "|";
                                    //token += ConfigurationHelper.Read("HospitalCode") + "|";
                                    //token += ConfigurationHelper.Read("EMRDept") + "|";
                                    ////token += deptId + "|";
                                    ////token += $"{ExtendApplicationContext.Current.LoginUserContext.DeptID}|";
                                    ////token += "|";
                                    //token += "|";
                                    //token += $"{ExtendApplicationContext.Current.PatientContext.PatientID}_{ExtendApplicationContext.Current.PatientContext.VisitID}|";
                                    ////token += patId + "|";
                                    //token += "|";
                                    //token += (DateTime.Now.Ticks / 10000).ToString();
                                    //Logger.Write($"电子病历访问，token:{token}");
                                    //byte[] tokenByte = Encoding.Default.GetBytes(token);
                                    //string tokenBase64 = Convert.ToBase64String(tokenByte);
                                    //webPath += tokenBase64;
                                    //Process.Start(webPath);
                ...
                else if (e.ViewName == ViewNames.InOperation)  //术中登记
                {
                    if (IsMatchingUser() == 0)
                    {
                        Dialog.MessageBox("提示：该病人麻醉医生不是登录用户。");
                        return;
                    }
                    else if (IsMatchingUser() == 1)
                    {
                        Dialog.MessageBox("提示：该病人麻醉医生不是登录用户。");
                    }
                    if (ExtendApplicationContext.Current.PatientInformation == null)
                ...
                {
                    LoginAnes loginForm = new LoginAnes(false);
                ...
                {
                    var s = ExtendApplicationContext.Current;
                    string usename = ExtendApplicationContext.Current.LoginUserContext.LoginName;
                    string pwd = ExtendApplicationContext.Current.LoginUserContext.PWD;
                    Process run = new Process();
                    run.StartInfo.FileName = ApplicationConfiguration.AnesQueryAddress+"?UserName="+ usename+"&Pwd="+ pwd;
                    run.Start();
      ...
      , "信息", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2, 0) == DialogResult.No)
                ...
                else if (e.ViewName == ViewNames.SyncHis)//HIS同步
                {
                    if (ExtendApplicationContext.Current.CustomSettingContext.IsSyncScheduleInfo)
                    {
                        ...
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(ExtendApplicationContext.Current.PatientContext.PatientID))
                            {
                                Logger.Write(DateTime.Now.ToString() + "主界面开始同步检验:" + ExtendApplicationContext.Current.PatientContext.PatientID);
                                (new SyncDA()).SyncLis(ExtendApplicationContext.Current.PatientContext.PatientID, null);
                            }
                            else
                            {
                                Logger.Write("未同步，未选择患者");
                            }
                ...
                else if (e.ViewName == ViewNames.Mzzj)//麻醉总结
                {
                    MzzjControl1 mzzj = new MzzjControl1();
                    mzzj.Caption = "麻醉总结";
                    this.workSpaceControl1.ShowViewDialog(mzzj, 860, 600);

                }
                else if (e.ViewName == ViewNames.BISApply)//输血申请
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(ExtendApplicationContext.Current.PatientInformation.PatientID))
                        {
                            //web访问输血申请
                            string webPath = ConfigurationHelper.Read("BISWebPath");
                            if (!string.IsNullOrEmpty(webPath))
                            {
                                string token = "";
                                token += $"&username={ExtendApplicationContext.Current.LoginUserContext.UserID}|";
                                token += $"&password={ExtendApplicationContext.Current.LoginUserContext.PWDBase64}|";
                                token += $"&patient_id={ExtendApplicationContext.Current.PatientInformation.PatientID}|";
                                token += $"&dept_id={ExtendApplicationContext.Current.LoginUserContext.DeptID}|";

                                Logger.Write($"输血申请访问，token:{token}");
                                webPath += token;
                                Process.Start(webPath);
                            }
                        }
        ...
        public void ClearMenu(object menuContainer, object subMenuContainer)
        {
            if ((menuContainer as Control).Controls != null)
            {
                (menuContainer as Control).Controls.Clear();
            }

            if ((subMenuContainer as HorizontalNavBar).Controls != null)
            {
                (subMenuContainer as HorizontalNavBar).ClearItems();
            }
        }
        /// <summary>
        /// 
        /// </summary>
        /// <returns>0不可以编辑提示 1可以编辑提示 2可以编辑</returns>
        private int IsMatchingUser()
        {
            ...
            if (loginUser.ToUpper() == "ADMIN" || loginUser.ToUpper() == "MZK" || loginUser.ToUpper() == "ZZS"||!ApplicationConfiguration.IsPACUProgram)
            {
                result = 2;
            }
            else
            {
                string name = ExtendApplicationContext.Current.LoginUserContext.UserName;//登录医生姓名
                AnesInformations.OperationMasterDataTable operationMasterDataTable = AnesthesiaSheetProxy.GetOperationMaster(ExtendApplicationContext.Current.PatientInformation.PatientID, ExtendApplicationContext.Current.PatientInformation.VisitID, ExtendApplicationContext.Current.PatientInformation.OperID);

                string name1 = ExtendApplicationContext.Current.PatientInformation.AnesDoctor;//病人麻醉医生姓名

                //string name1=GetHisUserName(operationMasterRow.ANES_DOCTOR, this._doctorTable);
                string codetoname = GetHisUserName(operationMasterDataTable[0]["ANES_DOCTOR"].ToString(), this._doctorTable);
                string codetoname1 = GetHisUserName(operationMasterDataTable[0]["SECOND_ANES_DOCTOR"].ToString() , this._doctorTable);
                string codetoname2 = GetHisUserName(operationMasterDataTable[0]["THIRD_ANES_DOCTOR"].ToString(), this._doctorTable);
                string code1 = "";
                string code2 = "";
                if (name1==""&& code1==""&& code2 == "")
                {
                    result = 2;
                }
                else
                {
                    name1 = codetoname == "" ? name1 : codetoname;
                    if (!string.IsNullOrWhiteSpace(operationMasterDataTable[0]["SECOND_ANES_DOCTOR"].ToString()))
                        code1 = codetoname1 == "" ? operationMasterDataTable[0]["SECOND_ANES_DOCTOR"].ToString() : codetoname1; 
                    if (!string.IsNullOrWhiteSpace(operationMasterDataTable[0]["THIRD_ANES_DOCTOR"].ToString()))
                        code2 = codetoname2==""? operationMasterDataTable[0]["THIRD_ANES_DOCTOR"].ToString(): codetoname2;

                    if (name == name1 || name == code1 || name == code2)
                    {
                        result = 2;
                    }
                    else
                    {
                        if (Int32.Parse(operationMasterDataTable[0]["OPER_STATUS"].ToString())>=35)
                        {
                          result = 0;
