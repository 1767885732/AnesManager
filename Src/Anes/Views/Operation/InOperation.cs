...
namespace Wis.Anes.Views
{
    ...
    /// </summary>
    [ToolboxItem(false)]
    public class InOperation : BaseView
    {
        /// <summary>
        /// 构造方法
        /// </summary>
        /// <param name="patientInfo">病人基本信息</param>
        public InOperation(PatientInformation patientInfo, decimal eventNo):this(patientInfo,eventNo,false)
        {
        }

        public InOperation(PatientInformation patientInfo, decimal eventNo, bool showAnesAndOper)
        {
            ...
            _anesthesiaEventDictSelector = new AnesthesiaEventDictSelector(eventNo, showAnesAndOper);
            _anesthesiaEventDictSelector.Dock = DockStyle.Right;
            _anesthesiaEventDictSelector.ItemSelected += new EventHandler(_anesthesiaEventDictSelector_ItemSelected);
            //_anesthesiaEventDictSelector.AddAnes += new EventHandler(_anesthesiaEventDictSelector_AddAnes);
            //_anesthesiaEventDictSelector.AddOper += new EventHandler(_anesthesiaEventDictSelector_AddOper);
            pnlBody.Controls.Add(_anesthesiaEventDictSelector);

            _anesthesiaEventsEditor = new AnesthesiaEventsEditor(patientInfo, eventNo);
            ...
            _patMonitorEditor = new PatMonitorEditor(patientInfo.PatientID, patientInfo.VisitID, patientInfo.OperID, eventNo);
            ...
            if (ExtendApplicationContext.Current.AppType == ApplicationType.PACU)
            {
                ...
                if (_anesthesiaEventDictSelector != null)
                {
                    _anesthesiaEventDictSelector.Visible = true;
                }
            ...
            {
                if (!Framework.AccessControl.CheckModifyRight(PermissionContext.ANESRECORDOPER) || !Framework.AccessControl.CheckModifyRightForOperator("麻醉记录单"))//ApplicationConfiguration.AnesRecordOper)//麻醉数据修改操作权限控制
                {
                    if (_anesthesiaEventsEditor != null)
                    {
                    ...
                    if (_anesthesiaEventDictSelector != null)
                    {
                        _anesthesiaEventDictSelector.Visible = false;
                    }
                ...
                if (!Framework.AccessControl.CheckModifyRight(PermissionContext.MonitorDataEdit) || !Framework.AccessControl.CheckModifyRightForOperator("麻醉记录单"))
                {
                    if (_patMonitorEditor != null)
                    {
            ...
            if (ExtendApplicationContext.Current.EventNo == 1 )
            {
                if (!ExtendApplicationContext.Current.CustomSettingContext.IsShowAnesthesiaEventsEditor)
                {
                    ...
                    SetAnesthesiaEventDictSelectorVisible(false);
        ...
        //void _anesthesiaEventDictSelector_AddOper(object sender, EventArgs e)
        //{
        //    if (_anesthesiaEventsEditor != null)
        //    {
        //        _anesthesiaEventsEditor.AddOper();
        //    }
        //}

        //void _anesthesiaEventDictSelector_AddAnes(object sender, EventArgs e)
        //{
        //    if (_anesthesiaEventsEditor != null)
        //    {
        //        _anesthesiaEventsEditor.AddAnes();
        //    }
        //}
        ...
        private void _anesthesiaEventDictSelector_ItemSelected(object sender, EventArgs e)
        {
            ...
            {
                AnesInformations.AnesthesiaEventRow row = AddRow(sender);
                if (_anesthesiaEventsEditor != null)
                {
        ...
        private AnesthesiaEventDictSelector _anesthesiaEventDictSelector = null;
        ...
        /// </summary>
        /// <param name="visible"></param>
        public void SetAnesthesiaEventDictSelectorVisible(bool visible)
        {
            if (_anesthesiaEventDictSelector != null)
            {
                _anesthesiaEventDictSelector.Visible = visible;
            }
        }
