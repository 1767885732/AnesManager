/*----------------------------------------------------------------
      ...
      // 文件名：SqlScriptProvider.cs
      // 文件功能描述：SqlScriptProvider
...
namespace Wis.Anes.DataAccess
{
    public class SqlScriptProvider
    {
            ...
            get
            {
                return
                    "SELECT PAT_ID, VISIT_ID, OPER_ID, OPER_PART, ALERGY1, ALERGY2,  PRE_OPER_CONS, PRE_ANES_PHAM, PRE_ANES_PHAM_RESULT, PRE_OPER_SKIN_STATUS, "
                     ...
                     + "  D_PRESS, S_IN_TIME, S_OUT_TIME, S_PRESS, URETER_SIZE, IMPLANT_NAME, IMPLANT_MANUFACTORY, IMPLANT_OUT, SEND_IMPLANT_TO "
        ...
        public string Dict_GetHisUserByUserDept
        {
            get
            {
                return
                   " SELECT USER_NAME, USER_ID, USER_JOB, USER_DEPT, RESERVED01, CREATE_DATE_TIME,INPUT_CODE FROM WIS_PERM_HIS_USER WHERE (USER_DEPT = @userdept)";
            }
        }
        ...
        public string MyDataSet_GetPermissionByLoginName
        {
            get
            {
                return
                   ...
                   + " WHERE A.APP_ID = B.APP_ID AND B.APP_ID = @APP_ID AND A.PERMISSION_ID = C.PERMISSION_ID AND D.ROLE_ID = C.ROLE_ID AND D.USER_ID = E.USER_ID  AND E.LOGIN_NAME = @LoginName";
            }
        }
        ...
        public string PatientBaseInformations_GetPatMasterIndexByPatient
        {
            ...
            {
                return
                   "SELECT DATEDIFF(YEAR, DATE_OF_BIRTH, GETDATE()) AS AGE, BIRTH_PLACE, CHARGE_TYPE, CITIZENSHIP, CREATE_DATE_TIME, DATE_OF_BIRTH, [IDENTITY], ID_NO, INP_NO, LAST_VISIT_DATE, MAILING_ADDRESS, NAME, NAME_PHONETIC, NATION, "
                   + "NEXT_OF_KIN, NEXT_OF_KIN_ADDR, NEXT_OF_KIN_PHONE, NEXT_OF_KIN_ZIP_CODE, OPERATOR, PAT_ID, PHONE_NUMBER_BUSINESS, PHONE_NUMBER_HOME, RELATIONSHIP, SEX, UNIT_IN_CONTRACT, VIP_INDICATOR, ZIP_CODE FROM "
                   + "WIS_PAT_MASTER_INDEX WHERE (PAT_ID = @PatientID)";
            }
        }
        ...
        #region CareDocs
        public string CareDocs_GetCustomFieldDictTable
        {
            get
            {
                return
        ...
        public string CareDocs_GetTableDescTable
        {
            get
            {
                return " select * from WIS_DICT_CUSTOM_FIELD WHERE FIELD_DESC ='table' ";
            }
        }
        public string CareDocs_GetFieldDescTable
        {
            get
            {
                return " select * from WIS_DICT_CUSTOM_FIELD WHERE FIELD_DESC =@FIELD_DESC ";
            }
        }
        ...
        public string LocalDataModelProxy_WIS_PAT_MONITOR_DATA_HISTORY
        {
            get
            {
                return
        ...
        public string Analyze_GetAnesthesiaMethod
        {
            get
            {
                return "  SELECT * FROM ( SELECT TOP 100 PERCENT ISNULL( A.ANES_METHOD ,'未知' ) AS 麻醉方法  ,COUNT(1) AS 台数  FROM WIS_OPER_MASTER A "
                + " WHERE A.START_DATE_TIME>=@START_DATE_TIME AND A.START_DATE_TIME<=@END_DATE_TIME AND A.OPER_STATUS >=2 "
                + " GROUP BY ANES_METHOD  ORDER BY 台数 DESC ) X "
                + " UNION ALL "
                + " SELECT '总计'AS 麻醉方法 , SUM(台数) 台数  FROM ( "
                + " SELECT ISNULL( A.ANES_METHOD ,'未知' ) AS 麻醉方法  ,COUNT(1) AS 台数  FROM WIS_OPER_MASTER A "
                + " WHERE A.START_DATE_TIME>=@START_DATE_TIME AND A.START_DATE_TIME<=@END_DATE_TIME AND A.OPER_STATUS >=2 "
                + " GROUP BY ANES_METHOD ) Y";
            }
        }
        public string Analyze_GetASAGrade
        {
            get
            {
                return "  SELECT * FROM "
                + " ( "
                + " SELECT TOP 100 PERCENT ASA_GRADE  级别 , COUNT(*) AS 数量 FROM  "
                + " (  "
                + " SELECT  "
                + " (  "
                + " ISNULL(  B.ASA_GRADE ,'未填' )   "
                + " ) AS ASA_GRADE  "
                + " FROM WIS_OPER_MASTER A LEFT JOIN WIS_ANES_PLAN B  "
                + " ON A.PAT_ID = B.PAT_ID AND A.VISIT_ID = B.VISIT_ID AND A.OPER_ID = B.OPER_ID  "
                + " WHERE  A.START_DATE_TIME>=@START_DATE_TIME AND A.START_DATE_TIME<=@END_DATE_TIME  "
                + " ) W GROUP BY ASA_GRADE ORDER BY 级别 "
                + " ) X "
                + " UNION ALL  "
        ...
        public string Analyze_GetThreeReview
        {
            get
            {
                ...
                + " ( "
                + "   SELECT B.PAT_ID,B.OPER_ID,B.ITEM_NAME,B.ITEM_VALUE FROM  "
                + "   ( "
        ...
        public string Analyze_GetInPACUCount
        {
            get
            {
                ...
                + " AND A.START_DATE_TIME>=@START_DATE_TIME AND A.START_DATE_TIME<=@END_DATE_TIME  ";
            }
        }
        ...
        public string Analyze_SheBeiYiQi
        {
            get
            {
                ...
                + "   T1.ITEM_NAME AS 设备名称  ,T1.ITEM_BRAND AS 品牌 ,T1.ITEM_SPEC AS 设备型号 ,ISNULL(T2.USECOUNT,0) AS 使用次数  FROM  "
                ...
                + "       ON B.PAT_ID = A.PAT_ID AND B.VISIT_ID = A.VISIT_ID AND B.OPER_ID = A.OPER_ID  "
                + "       WHERE ITEM_NAME LIKE '手术护理单_设备仪器%' AND  ISNULL(ITEM_VALUE,'##') != '##'  "
                + "       AND A.START_DATE_TIME>=@START_DATE_TIME AND A.START_DATE_TIME<=@END_DATE_TIME   "
                + "     ) GROUP BY ITEM_VALUE "
                + "   ) T2 "
