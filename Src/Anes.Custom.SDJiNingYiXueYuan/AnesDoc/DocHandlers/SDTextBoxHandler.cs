...
namespace Wis.Anes.Custom.CustomProject.AnesDocHandlers
{
    public class SDTextBoxHandler : TextBoxHandler
    {
        ...
        public override void BindDataToUI(MTextBox control, Dictionary<string, DataTable> dataSources)
        {
            ...
            if (!string.IsNullOrEmpty(control.SummaryFormula))
            {
                DataContext.GetCurrent().CalTextValueFormula(control, dataSources);
            }
            else if (!string.IsNullOrEmpty(control.SummaryName) && control.SummaryName.Trim() != string.Empty)
            {
                if ((control.SummaryName.Contains("时长") || control.SummaryName.Contains("时间")))
                {
                    DataContext.GetCurrent().CalTimeRange(control, dataSources);
                }
                else if (control.SummaryName.Contains("实时") || control.SummaryName.Contains("当前"))
                {
                    DataContext.GetCurrent().CalCurrentVitalSign(control, dataSources, "DESC");
                }
                else if (control.SummaryName.Contains("首例") || control.SummaryName.Contains("首次"))
                {
                    DataContext.GetCurrent().CalCurrentVitalSign(control, dataSources, "ASC");
                }
                else
                {
            ...
            this.HasDirty = false;
            //if (!string.IsNullOrEmpty(control.SummaryName) && control.SummaryName.Trim() != string.Empty
            //     && control.SummaryName.Trim() != "总入量" && control.SummaryName.Trim() != "总出量") //&& string.IsNullOrEmpty(control.Text.Trim()))
            //{
            //    if (control.SummaryName.Trim() != "尿量" && control.SummaryName.Trim() != "失血量")//add by dhc 20200610 冠县中心医院不需要自动获取出量
            //    {
            //        DataContext.GetCurrent().CalTextValue(control, dataSources);
            //    }                               
            //}
