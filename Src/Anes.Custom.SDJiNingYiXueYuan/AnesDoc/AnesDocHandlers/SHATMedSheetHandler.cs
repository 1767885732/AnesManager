...
namespace Wis.Anes.Custom.CustomProject
{
    public class SHATMedSheetHandler : UIElementHandler<MedSheet>
    {
        ...
        public override void BindDataToUI(MedSheet control, Dictionary<string, System.Data.DataTable> dataSources)
        {
            ...
            DataTable dtMonitorEx = dataSources["WIS_PAT_MONITOR_DATA_EXT"];
            ...
            foreach (Wis.Anes.Framework.Doc.IUIElementHandler handler in MedicalPaperUIElementHandlers)
            {
            ...
            foreach (MedVitalSignCurveDetail obj in list)
            {
                ...
                foreach (System.Reflection.PropertyInfo propertyInfo in propertyInfos)
                {
                    row[propertyInfo.Name] = Wis.Anes.Framework.Controls.Base.AssemblyHelper.GetPropertyValue(propertyInfo, obj);
                }
            ...
            //{
            //    string deleteSql = "delete from WIS_PATIENT_FILL_MONITORING WHERE 病人号='" + ExtendApplicationContext.Current.PatientContext.PatientID + "' AND 住院号=" + ExtendApplicationContext.Current.PatientContext.VisitID +
            //                       " AND 手术号=" + ExtendApplicationContext.Current.PatientContext.OperID;
            //    cda.ExecuteNonQuery(deleteSql);
            //    string insertSql = "insert into  WIS_PATIENT_FILL_MONITORING (病人号,住院号,手术号,监测项目,备注,创建人,创建日期) " +
            ...
            for (int n = 0; n < control.Rows.Count; n++)
            {
                ...
                string itemCode = string.IsNullOrEmpty(item.ItemCode) ? item.ItemName : item.ItemCode;
                DataRow[] rows = null;
                DataRow[] dtMonitorExrows = null;

                if (item.IsGdShow && !string.IsNullOrEmpty(item.ItemName))
                {
                    #region 这部分代码是自动从个性化表中加载过来的数据．
                    //by dhc 2020/3/11
                    if (GetStringValue(item.ItemCode).Trim().Length == 0 && !string.IsNullOrEmpty(arryFillMonitoringList[0]))
                    {
                        item.ItemName = GetStringValue(arryFillMonitoringList[ifiimonStep]);
                        if (GetStringValue(item.ItemName).Trim().Length != 0)
                        {
                            #region 拼装数据
                            dtak880.Clear();
                            dtak880.Dispose();

                            dtMonitorExrows = dtMonitorEx.Select("ITEM_NAME = '" + item.ItemName + "' and TIME_POINT<='" + endTime + "'", "TIME_POINT ASC");
                            ...
                            rows = vitalSignData.Select("ITEM_NAME = '" + item.ItemName + "' and TIME_POINT>='" + startTime + "' and TIME_POINT<='" + endTime + "'", "TIME_POINT ASC");
                            if (rows.Length != 0)
                            {
                    ...
                    string itemName = item.ItemName.Trim().ToUpper();

                    // ECG等麻醉机采集或手动输入项目
                    if (itemName == "ECG" || itemName == "尿量" || itemName == "累计失血量" || itemName == "TVE" || itemName == "VTE" || itemName == "PPLAT"
                        || itemName == "I:E" || itemName == "F" || itemName == "PEEP")
                    {
                        rows = dtMonitorEx.Select("ITEM_CODE = '" + item.ItemCode + "' and TIME_POINT<='" + endTime + "'", "TIME_POINT ASC");
                    }
                    // SP02等监护仪采集项目
                    else if (itemName == "SPO2" || itemName == "CVP" || itemName == "ETCO2")
                    {
                        #region 拼装数据
                        dtak880.Clear();
                        dtak880.Dispose();
                     ...
                     ***********************************************************************************************/
                    //麻醉机或手动输入项目
                    if (itemName == "ECG" || itemName == "尿量" || itemName == "累计失血量"
                        || itemName == "TVE" || itemName == "VTE" || itemName == "PPLAT"
                        || itemName == "I:E" || itemName == "F" || itemName == "PEEP")
                    {
                        //分别计算每行每列的值
                        for (int i = 0; i < control.Cols.Count; i++)
                    ...
                    //  SP02  或者  EtCO2 等监护仪采集的在这里处理的
                    else if (itemName.Equals("SPO2") || itemName.Equals("ETCO2") || itemName == "CVP")
                    {
                    ...
                    {
                        ////均未采集到数据（或手写数据）
                        //if (vitalSignData.Select("ITEM_NAME = '" + item.ItemName + "' and TIME_POINT>='" + startTime + "' and TIME_POINT<='" + endTime + "'", "TIME_POINT ASC").Length == 0
                        //    && dtMonitorEx.Select("ITEM_NAME = '" + item.ItemName + "' and TIME_POINT<='" + endTime + "'", "TIME_POINT ASC").Length == 0)
                        //{
                ...
                else  // 为空的时候,自动加载
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(arryFillMonitoringList[0]))
                        {
                            item.ItemName = GetStringValue(arryFillMonitoringList[ifiimonStep]);
        ...
        private void MedSheetShowSet()
        {
            ...
            if (result != null && (DialogResult)result == DialogResult.OK)
            {
                ...
                Configuations.PatientMonitorConfigDataTable configTable = configurationDA.GetPatientJianCeConfigDataTable(
        ...
        protected void EditValue(MedSheetCell cell)
        {
            ...
            if (!string.IsNullOrEmpty(row.DictTableName) && !string.IsNullOrEmpty(row.DictValueFieldName))
            {
                ...
                DataRow[] rows = BuildPopupItemsDataWithEmpty(row.DictTableName, row.DictWhereString);//增加一个置空选项
        ...
        private void TextBox_LostFocus(object sender, EventArgs e)
        {
            ...
            if (_textBox.Tag != null)
            {
                try
                {
                    ...
                    {
                        cell.Value = _textBox.Text=="置空"?"" : _textBox.Text;
        ...
        {
            DataTable dtMonitorEx = base.DataSource["WIS_PAT_MONITOR_DATA_EXT"];
            dtMonitorEx.TableName = "WIS_PAT_MONITOR_DATA_EXT";
            ...
            {
                string itemname = "尿量";
                DataRow[] rows = anesEventTable.Select("ITEM_NAME = '" + itemname + "' and START_DATE_TIME='" + cell.TimePoint + "'", "START_DATE_TIME");
                if (rows.Length == 0 && !string.IsNullOrEmpty(_textBox.Text))
                {
                    ...
                    eventRow.START_DATE_TIME = cell.TimePoint;
            ...
            else
            {
                ...
                if (rows.Length == 0 && !string.IsNullOrEmpty(_textBox.Text))
                {
                    ...
                    row["RECORD_DATE"] = System.DateTime.Now;
                ...
                else if (rows.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < rows.Length; i++)
                        {
                            if (i == 0)  //周青
                            {
                                rows[i]["ITEM_VALUE"] = _textBox.Text=="置空"?"": _textBox.Text;
                                rows[i]["RECORD_DATE"] = System.DateTime.Now;
                                //if(_textBox.Text== "置空")
                                //{
                                //    dtMonitorEx.Rows.Remove(rows[i]);
                                //}
