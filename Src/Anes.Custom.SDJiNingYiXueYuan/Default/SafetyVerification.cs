...
{
    public partial class SafetyVerification : CustomBaseDoc
    {
        public SafetyVerification()
        {
            InitializeComponent();
        ...
        protected override void BuildData(Dictionary<string, DataTable> dataSource)
        {
            ...
            List<CustomControl> ctls = this.GetControls<CustomControl>();
            foreach (CustomControl ctl in ctls)
            {
                if (ctl.Tag != null && ctl.Tag.ToString().Trim() == "术中")
                {
                ...
                else if (ctl.Tag != null && ctl.Tag.ToString().Trim() == "术后")
                {
                    if (ExtendApplicationContext.Current.OperationStatus < OperationStatus.OperationEnd)
                    {
                        ctl.ReadOnly = true;
                    }
                }
