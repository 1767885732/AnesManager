...
namespace Score.Common.Controls
{
    public partial class Lutz : BaseControl
    {
        ...
        private Wis.Anes.BusinessEntity.Score.MED_LUTZ_SCORING_RESULTDataTable lutzDataTable;
        private Wis.Anes.BusinessEntity.Score.MED_LUTZ_SCORING_RESULTDataTable lutzTable;
        ...
        private void bindToDataGrid()
        {
            ...
            dgvLutz.Columns[0].DataPropertyName = patientScore.Columns["SCORING_DATE_TIME"].ColumnName;
            dgvLutz.Columns[1].DataPropertyName = patientScore.Columns["SCORING_VALUE"].ColumnName;
            dgvLutz.Columns[2].DataPropertyName = patientScore.Columns["degree"].ColumnName;
            dgvLutz.Columns[3].DataPropertyName = patientScore.Columns["MEMO"].ColumnName;
            dgvLutz.Columns[4].DataPropertyName = patientScore.Columns["OPERATOR"].ColumnName;
            RefreshGraph();
        }
        /// <summary>
        /// 刷新信息
        ...
        private int saveDetail()
        {
            ...
            lutzDataTable = new Wis.Anes.BusinessEntity.Score.MED_LUTZ_SCORING_RESULTDataTable();
            Wis.Anes.BusinessEntity.Score.MED_LUTZ_SCORING_RESULTRow row = lutzDataTable.NewMED_LUTZ_SCORING_RESULTRow();
            row.PAT_ID = _patientID;
            row.VISIT_ID = _visitID;
            //row.DEP_ID = _deptID;
            scoreTime = DataOperator.GetSysDate();
            
            row.SCORING_DATE_TIME = scoreTime;
            row.S1 = cbs1.SelectedIndex;
            row.S2 = cbs2.SelectedIndex;
            row.S3 = cbs3.SelectedIndex;
            row.S4 = cbs4.SelectedIndex;
            row.S5 = cbs5.SelectedIndex;
            row.S6 = cbs6.SelectedIndex;
            row.S7 = cbs7.SelectedIndex;
            row.S8 = cbs8.SelectedIndex;

            row.S9 = cbs9.SelectedIndex;
            row.S10 = cbs10.SelectedIndex;
            row.S11 = cbs11.SelectedIndex;
            row.S12 = cbs12.SelectedIndex;
            row.S13 = cbs13.SelectedIndex;
            row.S14 = cbs14.SelectedIndex;
            row.S15 = cbs15.SelectedIndex;

            row.MEMO = this.txtMemo.Text;
            lutzDataTable.Rows.Add(row);
        ...
        private int saveScoreResult()
        {
            ...
            addRow.SCORING_DATE_TIME = scoreTime;
            addRow.SCORING_METHOD = "Lutz";
        ...
        private void pushValue(Wis.Anes.BusinessEntity.Score.MED_LUTZ_SCORING_RESULTDataTable lutzDataTable)
        {
            cbs1.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S1"].ToString().Trim());
            cbs2.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S2"].ToString().Trim());
            cbs3.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S3"].ToString().Trim());
            cbs4.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S4"].ToString().Trim());
            cbs5.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S5"].ToString().Trim());
            cbs6.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S6"].ToString().Trim());
            cbs7.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S7"].ToString().Trim());
            cbs8.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S8"].ToString().Trim());

            cbs9.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S9"].ToString().Trim());
            cbs10.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S10"].ToString().Trim());
            cbs11.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S11"].ToString().Trim());
            cbs12.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S12"].ToString().Trim());
            cbs13.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S13"].ToString().Trim());
            cbs14.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S14"].ToString().Trim());
            cbs15.SelectedIndex = int.Parse(lutzDataTable.Rows[0]["S15"].ToString().Trim());
            txtScore.Text = dgvLutz.SelectedRows[0].Cells["scoreValue"].Value.ToString();
            txtMemo.Text = patientScore.Rows[0]["MEMO"].ToString();
            txtdangerLevel.Text = patientScore .Rows[0]["degree"].ToString();
        ...
        private int findMainTableIndex()
        {
            ...
            for (index = dgvLutz.SelectedRows[0].Index + deleteBeforeSelect; index < patientScore.Rows.Count; index++)
            {
        ...
        {
            for (int rowNo = 0; rowNo < lutzTable.Rows.Count; rowNo++)
            {
                if (lutzTable.Rows[rowNo].RowState != DataRowState.Deleted)
                {
                    if (lutzTable.Rows[rowNo]["SCORING_DATE_TIME"].ToString() == patientScore.Rows[findMainTableIndex()]["SCORING_DATE_TIME"].ToString())
                        return rowNo;
                }
            }
            return -1;
     ...
     /// <param name="e"></param>
        ...
        private void btn_dele_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lutzTable.Rows[quaryIndex()].Delete();
