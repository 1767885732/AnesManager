...
namespace Score.Common.Controls
{
    ...
    public partial class ScoreAPACHEIIPanel : BaseControl
    {
        ...
        /// 评分数据表
        /// </summary>
        private Wis.Anes.BusinessEntity.Score.PatientScoringResultDataTable appcheScore;
        private Wis.Anes.BusinessEntity.Score.Apache2ScoringResultDetailDataTable apache2Score;
        private Wis.Anes.BusinessEntity.CareItems.VitalSignsRecLifeDataTable careItemsDateTable;
        private Wis.Anes.BusinessEntity.CheckReport.AssayReportDataTable AssayTable;
        ...
        private void RefreshGraph()
        {
            ...
            if (appcheScore != null)
            {
                foreach (Wis.Anes.BusinessEntity.Score.PatientScoringResultRow row in appcheScore)
                {
        ...
        private void setValue(Wis.Anes.BusinessEntity.Score.Apache2ScoringResultDetailRow row, string text, string columnName)
        {
            decimal value = 0;
            try
            {
        ...
        private int saveScoreResult()
        {
            ...
            Wis.Anes.BusinessEntity.Score.PatientScoringResultRow addRow = (Wis.Anes.BusinessEntity.Score.PatientScoringResultRow)appcheScore.NewRow();
            addRow.PAT_ID = _patientID;
            addRow.VISIT_ID = _visitID;
            addRow.DEP_ID = _deptID;

            addRow.SCORING_DATE_TIME = scoreDateTime;
            addRow.SCORING_METHOD = "apache2";
            ...
            addRow.DEATH_PROBABILITY = (decimal)deatnRates;
        ...
        private int saveDetailSap()
        {
            ...
            Wis.Anes.BusinessEntity.Score.Apache2ScoringResultDetailDataTable detailTable = new Wis.Anes.BusinessEntity.Score.Apache2ScoringResultDetailDataTable();
            Wis.Anes.BusinessEntity.Score.Apache2ScoringResultDetailRow detailRow = (Wis.Anes.BusinessEntity.Score.Apache2ScoringResultDetailRow)detailTable.NewRow();
            detailRow.PAT_ID = _patientID;
            detailRow.VISIT_ID = _visitID;
            detailRow.DEP_ID = _deptID;
            scoreDateTime = DataOperator.GetSysDate();
            detailRow.SCORING_DATE_TIME = scoreDateTime;
            setValue(detailRow, txt_Age.Text, "AGE");
            setValue(detailRow, txt_HeartRate.Text, "HR");
            setValue(detailRow, txt_MAP.Text, "MAP");
            setValue(detailRow, txt_Breath.Text, "BR");
            setValue(detailRow, txt_Temperature.Text, "TMP");
            setValue(detailRow, txt_aD02.Text, "AADO2");
            setValue(detailRow, txt_Pa02.Text, "PAO2");
            setValue(detailRow, txt_Fi92.Text, "FIO2");
            setValue(detailRow, txt_PH.Text, "PH");
            setValue(detailRow, txt_HCT.Text, "HCT");
            setValue(detailRow, txt_WBC.Text, "WBC");
            setValue(detailRow, txt_K.Text, "K");
            setValue(detailRow, txt_Na.Text, "NA");
            setValue(detailRow, txt_Cr.Text, "CR");
            if (checkBox1.Checked == true)
                detailRow.KEY_INDICATOR = 1;
            if (radio_select.Checked == true)
                detailRow.HEALTH_STATUS = 2;
            else if (radio_NoSelect.Checked == true)
                detailRow.HEALTH_STATUS = 5;
            if (cbxJiZheng.Checked)
            {
                detailRow.JI_ZHEN_OPER = 1;
            }
            else
            {
                detailRow.SetJI_ZHEN_OPERNull();
            }

            if (cbxNoOper.EditValue != null)
            {
                detailRow.NO_OPER_PAT = cbxNoOper.SelectedText;
            }
            else
            {
                detailRow.SetNO_OPER_PATNull();
            }

            if (cbxAfterOper.EditValue != null)
            {
                detailRow.AFTER_OPER_PAT = cbxAfterOper.SelectedText;
            }
            else
            {
                detailRow.SetAFTER_OPER_PATNull();
            }
            detailRow.EYES_REFLECT = getEyeOpenValue();
            detailRow.TALK_REFLECT = getLanguageValue();
            detailRow.LIMB_REFLECT = getBodyValue();
            detailRow.MEMO = tex_DescriptionIllness.Text;
            detailTable.AddApache2ScoringResultDetailRow(detailRow);
        ...
        private void pushValue(Wis.Anes.BusinessEntity.Score.Apache2ScoringResultDetailDataTable deTable)
        {
            txt_Age.Text = deTable.Rows[0]["AGE"].ToString();
            txt_HeartRate.Text = deTable.Rows[0]["HR"].ToString();
            txt_MAP.Text = deTable.Rows[0]["MAP"].ToString();
            txt_Breath.Text = deTable.Rows[0]["BR"].ToString();
            txt_Temperature.Text = deTable.Rows[0]["TMP"].ToString();
            txt_aD02.Text = deTable.Rows[0]["AADO2"].ToString();
            txt_Pa02.Text = deTable.Rows[0]["PAO2"].ToString();
            txt_Fi92.Text = deTable.Rows[0]["FIO2"].ToString();
            txt_PH.Text = deTable.Rows[0]["PH"].ToString();
            txt_HCT.Text = deTable.Rows[0]["HCT"].ToString();
            txt_WBC.Text = deTable.Rows[0]["WBC"].ToString();
            txt_K.Text = deTable.Rows[0]["K"].ToString();
            txt_Na.Text = deTable.Rows[0]["NA"].ToString();
            txt_Cr.Text = deTable.Rows[0]["CR"].ToString();
            double death = double.Parse(dgvApache.SelectedRows[0].Cells["deathRate"].Value.ToString()) * 100;
            txtDeathRate.Text = death.ToString();
            txtScore.Text = dgvApache.SelectedRows[0].Cells["scoreValue"].Value.ToString();
            comb_EyeOpen.SelectedIndex = getEyeOpenIndex(int.Parse(deTable.Rows[0]["EYES_REFLECT"].ToString().Trim()));
            comb_language.SelectedIndex = getLanguageIndex(int.Parse(deTable.Rows[0]["TALK_REFLECT"].ToString().Trim()));
            comb_Body.SelectedIndex = getBodyIndex(int.Parse(deTable.Rows[0]["LIMB_REFLECT"].ToString().Trim()));
            tex_DescriptionIllness.Text = deTable.Rows[0]["MEMO"].ToString();
            checkBox1.Checked = getBoolValue(deTable.Rows[0]["KEY_INDICATOR"]);
            if (deTable.Rows[0]["HEALTH_STATUS"].ToString().Trim() != "")
            {
                if (double.Parse(deTable.Rows[0]["HEALTH_STATUS"].ToString().Trim()) == 2)
                    radio_select.Checked = true;
                if (double.Parse(deTable.Rows[0]["HEALTH_STATUS"].ToString().Trim()) == 5)
                    radio_NoSelect.Checked = true;
            }
            cbxJiZheng.Checked = getBoolValue(deTable.Rows[0]["JI_ZHEN_OPER"]);
            cbxNoOper.Text = deTable.Rows[0]["NO_OPER_PAT"].ToString();
            cbxAfterOper.Text = deTable.Rows[0]["AFTER_OPER_PAT"].ToString();
        }
        ...
        {
            for (int rowNo = 0; rowNo < apache2Score.Rows.Count; rowNo++)
            {
                if (apache2Score.Rows[rowNo].RowState != DataRowState.Deleted)
                {
                    if (apache2Score.Rows[rowNo]["SCORING_DATE_TIME"].ToString() == appcheScore.Rows[findMainTableIndex()]["SCORING_DATE_TIME"].ToString())
                        return rowNo;
                }
            }
            return -1;
        ...
        private void btn_dele_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    apache2Score.Rows[quaryIndex()].Delete();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(_patientID))
            {
                Wis.Anes.Framework.Utilities.Dialog.MessageBox("请先选中一个病人！");
                return;
            }
        ...
        private void dgvApache_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0)
                return;
            Wis.Anes.BusinessEntity.Score.Apache2ScoringResultDetailDataTable detailTable;
        ...
        private void setVital_lab_Value()
        {
            ...
            careItemsDateTable = new Wis.Anes.BusinessEntity.CareItems.VitalSignsRecLifeDataTable();
            string HeartRate = "心率";
            string MAP = "血压high";
            string Breath = "呼吸";
            ...
            //else
            {
                ...
                //    }
                //}
                //AssayTable = new Com.Wis.Icu.DataSetModel.CheckReport.AssayReportDataTable();
