    ...
    public partial class BaseControl : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void CheckChangedAndSave()
        {
            ...
            if (needSave)
            {
                ...
                if (Wis.Anes.Framework.Utilities.Dialog.MessageBox("数据已改变，是否保存？", _title, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    Save();
                }
                else
        ...
        protected virtual void OnPatientChange(Wis.Anes.BusinessEntity.Patient.PatientRow patientRow)
        {
            RefreshAll();
        }
