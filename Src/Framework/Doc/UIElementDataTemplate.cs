...
namespace Wis.Anes.Framework.Doc
{
  public class UIElementDataTemplate
    {
        ...
        public UIElementDataTemplate(BaseDoc baseDoc)
        ...
        public bool SaveModel()
        {
            ...
            //dataTable.TableName = "TOTALMODEL";
            //dataSet.Tables.Add(dataTable);
            //if (_sheetGrids != null && _sheetGrids.Count > 0)
            //{
            //    foreach (SheetGrid sheetGrid in _sheetGrids)
            //    {
            //        dataTable = sheetGrid.GetData();
            //        dataTable.TableName = sheetGrid.TableName;
            //        dataSet.Tables.Add(dataTable);
            //    }
            //}
            bool isSaved = false;
            ...
            //    {
            //        DataRow row = (DataRow)newDocumentTemplet.DialogResultData;
            //        row["DOCUMENT_NAME"] = Path.GetFileNameWithoutExtension(_baseDoc.ReportName);
            //        row["ISJUBU"] = 0;
            //        isSaved = SaveModel(row, FileHelper.StreamToBytes(DataSetToStream(dataSet)));
            //    }
            //}
            //else
            //{
            //    XtraMessageBox.Show("除患者基本信息外其他信息为空—请填写后再保存模板", "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //}
            return isSaved;
        }
        ...
        public bool SaveAllDataModel(DataTable TotalModelDT)
        {
            ...
            if (xtraForm.ShowDialog() == DialogResult.OK && newDocumentTemplet.DialogResultData != null)
            {
                Dict.DocumentTempletRow row = (Dict.DocumentTempletRow)newDocumentTemplet.DialogResultData;
                row.DOC_NAME = Path.GetFileNameWithoutExtension(_baseDoc.ReportName);
                row.IS_PART = 0;
        ...
        private bool SaveModel(Dict.DocumentTempletRow row)
        {
            Dict.DocumentTempletDataTable _documentTempletDataTable = row.Table as Dict.DocumentTempletDataTable;
            _documentTempletDataTable.AddDocumentTempletRow(row);
        ...
        private void SetQingdianViewData(string templetGuid,bool isAdd)
        {
            ...
            CareDocs.WIS_INSTRUMENT_TEMPLET_DETAILDataTable qiXieTempletDetail = (new CareDocsDA()).GetQiXieTempletDetailByGuid(templetGuid);
            if (qiXieTempletDetail != null && qiXieTempletDetail.Count > 0)
            {
                ...
                if (!isAdd)
                {
                    ...
                    {
                        int index = _currentGridView.Rows.Add();
                        ...
                        if (_currentGridView.MedGridViewColumns[i].HeaderText == _currentGridView.MedGridViewColumns[0].HeaderText)
                        {
                            for (int j = 0; j < _currentGridView.RowCount; j++)
                            {
                                if (_currentGridView[i, j].Value == null || (_currentGridView[i, j].Value != null && string.IsNullOrEmpty(_currentGridView[i, j].Value.ToString().Trim())))
                                {
                                    ...
                                    if (!prow.IsITEM_VALUENull() && (i + 1) < _currentGridView.MedGridViewColumns.Count)
                                    {
                                ...
                                {
                                    if (!prow.IsITEM_VALUENull() && !string.IsNullOrEmpty(prow.ITEM_VALUE.Trim()) && (i + 1) < _currentGridView.MedGridViewColumns.Count)
                                    {
        ...
        public  bool ApplyModel()
        {
            ...
            else if (_currentMRichTextBox != null)
            {
                if (_currentMRichTextBox.TempletFlag != DocTempletType.None)
                {
                    ...
                    {
                        documentTemplet = new DocumentTemplet(_currentMRichTextBox.OtherTempletFlagString.Trim(), ExtendApplicationContext.Current.EventNo);
                    }
                    else
                    {
                        documentTemplet = new DocumentTemplet(_currentMRichTextBox.TempletFlag, ExtendApplicationContext.Current.EventNo);
                    }
        ...
        public DataTable ApplyAllDataModel()
        {
            ...
            {
                //Dialog.MessageBox("请选择被套用控件");
                DocumentTemplet documentTemplet = new DocumentTemplet(DocTempletType.ALL, Path.GetFileNameWithoutExtension(_baseDoc.ReportName), ExtendApplicationContext.Current.EventNo);
                XtraForm xtraForm = GetDialogForm("套用模板", documentTemplet);
                if (xtraForm.ShowDialog() == DialogResult.OK)
                {
                    //_currentMRichTextBox.Text = documentTemplet.DialogResultData;
        ...
        private object ExecuteScalar(string sqlText)
        {
            CommonDA commonDA = new CommonDA();
            return commonDA.ExecuteScalar(sqlText);
        }
    }
}
