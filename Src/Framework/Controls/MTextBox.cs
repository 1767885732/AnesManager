/*----------------------------------------------------------------
      ...
      //
      // 
      // 重构标识：XXX-2008-01-16 优化结构 添加注释 扩展
----------------------------------------------------------------*/
using System;
...
namespace Wis.Anes.Framework.Controls
{
    ...
    public class MTextBox : TextBox, ICheckable, IPrintable//TextBox, ICheckable, IPrintable
    {
        ...
        [DisplayName("汇总项目公式(SummaryFormula)"), Description(@"录入功能示例：ITEM_CLASS = 'B' OR ITEM_CLASS = 'C',表示取事件表(WIS_ANESTHESIA_EVENT)ITEM_CLASS为B和C的所有剂量进行汇总！（当【汇总项目名称】也存在时，优先用【汇总项目公式】）"), Category("数据(自定义)")]
        public string SummaryFormula
        {
            get { return summaryFormula; }
            set { summaryFormula = value; }
        }

        // add by shiyu.duan at 20180918 for 主版本升级 anes-00090 麻醉方法下拉框可配置选择，增加删除功能 start
        private bool isSimple;
        [DisplayName("是否简易模式(IsSimple)"), Description(@"此配置只针对源数据字段名称为【ANESTHESIA_METHOD】或者【OPERATION_NAME】有效，用于设置下拉框是否为经典的简易模式"), Category("数据(自定义)")]
        ...
        protected string FormatDateTime(DateTime dateTime, string format)
        {
            ...
            else if (IsFunctionFormat(format))
            {
                if (format.ToLower().Trim().Equals("datetoage"))
                {
                    //Modify By xiasen.x@2014-05-29，获取服务器上的日期
                    ...
                    return Wis.Anes.Framework.Utilities.DateDiff.CalAge(dateTime, now);
                }
            }
...
{ 
    base.WndProc(ref   m);
    if (m.Msg == Wis.Anes.Framework.Utilities.WinAPI.WM_PAINT) 
    { 
