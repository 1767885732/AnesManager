...
{
    [ToolboxItem(false), Description("自定义控件")]
    public partial class CustomControl : UserControl, IPrintable
    {
        ...
        //[Description("最小列宽"), Category("数据(自定义)")]
        ...
        {
            if (this.SimpleValue != null) return;
            Font _font = this.Font;
            Controls.Clear();
            Control control = null;
            int top = 0, left = 0;
            int maxWidth = Width - xOffSet;
            if (_dataRows == null || _dataRows.Length < 1)
            {
                return;
            }
            for (int index = 0; index < _dataRows.Length; index++)
            {
                _displayFieldName = string.IsNullOrEmpty(_displayFieldName) ? "ITEM_NAME" : _displayFieldName;
                ...
                using (Graphics g = control.CreateGraphics())
                {
                    ...
                    {
                        left = 0;
                        top = index * ((int)g.MeasureString(text, _font).Height + 6);
                    }
                    else if (left + xOffSet + (int)g.MeasureString(text, _font).Width > Width)
                    {
                        left = 0;
                        top += (int)g.MeasureString(text, _font).Height + 6;
                    }
                    control.Top = top - 3;
                    control.Left = left;
                    control.Width = xOffSet + (int)g.MeasureString(text, _font).Width;
                   
                    left += xOffSet + (int)g.MeasureString(text, _font).Width;
                    */
        ...
        private void checkBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            rect.X = rect.X + xOffSet -1;

            rect.Width -= xOffSet - 1;
            ...
            using (SolidBrush solidBrush = new SolidBrush(checkBox.ForeColor))
            {
                ...
                g.DrawString(checkBox.Text, checkBox.Font, solidBrush, rect, sf);

                //int height = (int)g.MeasureString(checkBox.Text, checkBox.Font).Height;
                //g.DrawString(checkBox.Text, checkBox.Font, solidBrush, rect.X, rect.Y + 6);
            }
        }
        ...
        public void DrawGraphics(Graphics g, float x, float y)
        {
            ...
            //int index = 0;
            int checkSize = (int)Math.Ceiling(g.MeasureString(checkStr, Font).Width) - 5;
            ...
            foreach (Control ctl in Controls)
            {
                if (ctl is CheckBox)
                {
                    ...
                    {
                        g.DrawString(checkStr, Font, solidBrush, left, top);
                    }
                    rect.X = 2 + (int)left;
                    rect.Y = (int)top;
                    ...
                    rect.X = ctl.Location.X + (int)x;
                    rect.Y = ctl.Location.Y + (int)y;

                    rect.Width = checkSize;
                    rect.Height = checkSize;

                    if (_checkBoxLocation == LeftRightAlignment.Right)
                        rect.Offset(ctl.Width - xOffSet, 0);
                    ...
                    if ((ctl as CheckBox).Checked)
                    {
                        g.DrawString(checkStr, Font, solidBrush, rect.Left - 2, rect.Top);
                    }

                    // 绘制文字
                    if (_checkBoxLocation == LeftRightAlignment.Left)
                        rect.Offset(xOffSet - 1, -1);
                    else
                        rect.Offset(xOffSet - ctl.Width + 1, 1);
                    rect.Width = ctl.Width - xOffSet;
                    rect.Height = ctl.Height;


                    StringFormat sf = new StringFormat();
                    sf.Alignment = StringAlignment.Near;
                    g.DrawString(ctl.Text, Font, solidBrush, rect, sf);
                }
            }
            pen.Dispose();
            solidBrush.Dispose();
