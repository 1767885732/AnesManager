...
namespace Wis.Anes.Framework.Controls
{
    ...
    public partial class MedVitalSignGraph : AnesGraph,IPrintable
    {
        ...
        public override void DrawGraphics(Graphics graphics)
        {
            ...
            if (_drawRightBottom)
            {
                g.DrawLine(Pens.Black, rect.Right, OriginRect.Bottom - 1, OriginRect.Right, OriginRect.Bottom - 1);
            }
        ...
        private void DrawBloodGas(Graphics g)
        {
            ...
            {
                item.Recorddate = item.Recorddate.AddMinutes(5);
        ...
        {
            DataTable dt = ExtendApplicationContext.Current.CodeTables["WIS_DICT_BLOOD_GAS"];
            DataRow [] rows = dt.Select("BLG_CODE='" + itemcode + "'");
            if (rows.Length > 0)
                return rows[0].IsNull("BLG_UNITS")?"":rows[0]["BLG_UNITS"].ToString();
            else
                return "";
        }
        ...
        private double GetValue(float y1, MedVitalSignCurve curve)
        {
            ...
            float yRows = (y1 - _topOffSet - rectF.Y) / gridHeight;
            int yRow = (int)Math.Ceiling(yRows);
            int i = _yAxisScaleCount - 1 - yRow;
            if (i < 0) i = 0;
            int j = i;
            if (j >= _yAxisScaleCount) j--;
            double result = scaleValues[i].Value - (yRows - yRow) * (scaleValues[j + 1].Value - scaleValues[j].Value);
