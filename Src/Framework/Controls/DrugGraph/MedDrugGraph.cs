...
namespace Wis.Anes.Framework.Controls
{
    ...
    public partial class MedDrugGraph : AnesGraph,IPrintable
    {
        ...
        public override void DrawGraphics(Graphics graphics)
        {
            ...
            g.DrawLine(Pens.Black, rect.Left, 30, rect.Left, rect.Bottom);
        ...
        private void ResetLineHeight(Graphics g)
        {
            ...
            //_lineHeight = 1;
            //float h;
            ////float ph = Com.Wis.Common.Utilities.Globals.GetPrintRowHeight(_scaleRate, Font);
            ////if (ph > 0)
            ////{
            ////    h = ph + 1;
        ...
        private void DrawTotal(Graphics g, int index)
        {
            ...
            switch (_totalType)
            {
                case TotalType.SummeryValue:
                    ...
                    if (totalValue > 0)
                    {
                        g.DrawString(totalValue.ToString(), font, solidBrush, rect.X + rect.Width * (1 - _rightWidthPercent) + 5, rect.Top + index * (_lineHeight + 1) + 5 + _topOffSet);
                    }
                    break;
                case TotalType.Unit:
                    curve = _curves[index];
