...
namespace Wis.Anes.Framework.Controls
{
    ...
    public partial class MedAnesSheetDetails : AnesBand, IPrintable
    {
        ...
        private bool _hasOutLiquid = true;
        [Description("是否显示出量"), DisplayName("是否显示出量")]
        public bool HasOutLiquid
        {
            get
            {
                return _hasOutLiquid;
            }
            set
            {
                _hasOutLiquid = value;
        ...
        private bool _hasCommonDrug = true;
        [Description("是否显示用药"), DisplayName("是否显示用药")]
        public bool HasCommonDrug
        {
            get
            {
                return _hasCommonDrug;
            }
            set
            {
                _hasCommonDrug = value;
            }
        }
        ...
        public void DrawCollections(Graphics g)
        {
            ...
            //        {
            //            DrawCollectionText(g, collection.Text, collection.Color, x, y + (rowHeight - g.MeasureString("A", _detailFont).Height) / 2, columnWidth);
            //            rowIndex++;
            //            if (rowIndex >= rowCount)
            //            {
            //                rowIndex = 0;
            ...
            //                            DrawString(g, string.Format("{0}", point.Index), _detailFont, brush, x, y);
            //                            leftOffset = g.MeasureString("99", _detailFont).Width;
            //                        }
            //                        if (_hasTime)
            //                        {
            //                            drawText = point.StartTime.ToString("HH:mm") + ((point.PointType == PointType.ProLonged) ? point.EndTime.ToString(">HH:mm ")
            //                            : " ") + point.Text;
            //                        }
            //                        else
            //                        {
            ...
            //                        drawText = drawText + v;
            //                        DrawString(g, drawText, _detailFont, brush, x + leftOffset, y);
            //                        point.RectF = new RectangleF(x + leftOffset, y, g.MeasureString(drawText, _detailFont).Width, g.MeasureString(drawText, _detailFont).Height);
            //                        break;
            ...
            //                        else
            //                        {
            //                            drawText = point.Text + " " + point.Value.ToString() + point.Unit + " " + point.Route;
            //                        }
            //                        DrawString(g, drawText, _detailFont, brush, x + leftOffset, y);
            //                        point.RectF = new RectangleF(x + leftOffset, y, g.MeasureString(drawText, _detailFont).Width, g.MeasureString(drawText, _detailFont).Height);
            //                        break;
            //                    case CollectionType.Total:
            //                        DrawString(g, point.Text, _detailFont, ((point.Text.Equals(collection.Text) ? brush : new SolidBrush(_totalNormalColor))), rect.X + (columnCount - 1) * columnWidth + totalIndex * columnWidth / 2 + 5
            //                            , rect.Y + (rowCount - _totalRowCount + i) * rowHeight);
            //                        DrawString(g, point.Value.ToString(), _detailFont, new SolidBrush(_totalValueColor), rect.Right + totalIndex * columnWidth / 2 - g.MeasureString(point.Value.ToString() + " " + point.Unit, _detailFont).Width - 5 - columnWidth / 2
            //                            , rect.Y + (rowCount - _totalRowCount + i) * rowHeight);
            //                        DrawString(g, point.Unit, _detailFont, ((point.Text.Equals(collection.Text) ? brush : new SolidBrush(_totalNormalColor))), rect.Right + totalIndex * columnWidth / 2 - g.MeasureString(point.Unit, _detailFont).Width - 5 - columnWidth / 2
            //                            , rect.Y + (rowCount - _totalRowCount + i) * rowHeight);
            //                        break;
            //                }
            //                rowIndex++;
            ...
            //                    x = rect.X + columnWidth * columnIndex;
            //                }
            //                //if (g.MeasureString(drawText, _detailFont).Width > columnWidth) rowIndex++;
            //                y = rect.Y + rowIndex * rowHeight;
            //            }
            //    }
            //}
