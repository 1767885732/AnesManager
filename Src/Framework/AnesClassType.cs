...
namespace Wis.Anes.Framework
{
    ...
    public static class EventTypeHelper
    {
        ...
        public static Dict.AnesthesiaEventOpenDataTable RefreshEventOpenDataTalbe(string event_item_Class, string eventAttr)
        {
            DictDA _dictDA = new DictDA();
            if (string.IsNullOrEmpty(eventAttr))
            {
                return _dictDA.GetAnesthesiaEventOpen(event_item_Class);
            }
            else if (event_item_Class == "9" && eventAttr == "4")//9为呼吸
            {
                return _dictDA.GetAnesthesiaEventOpenByhuxi();
            }
            else
            {
                return _dictDA.GetAnesthesiaEventOpen(event_item_Class, eventAttr);
            }
        }
