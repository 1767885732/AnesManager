    ...
    public partial class LabValueTrendChart : BaseView
    {
        ...
        private Bitmap DrawingImg()
        {
            ...
            if (points.Count > 0)
            {
                //if (points.Count == 1)
                //{
                //    pf.X = (float)(intLeft + intXScale);
                //    g.RotateTransform(90f);
                //    g.DrawString(points[0].Key, new Font("宋体", 12), Brushes.Black, pf);
                //    g.RotateTransform(-90f);
                //}
                ...
                for (int i = 0; i < points.Count ; i++)
                {
                    ...
                    //KeyValuePair<string, double> keyValue2 = points[i + 1];
                ...
                {
                    //g.DrawCurve(new Pen(Color.Red), pointlist.ToArray(),0.2f);
        ...
        public LabValueTrendChart(string reportItemName)
        {
            InitializeComponent();
        ...
        private void LabValueTrendChart_Load(object sender, EventArgs e)
        {
            ...
            string sql = string.Format(@"SELECT A.REPORT_ITEM_NAME, A.RESULT,A.RESULT_DATE_TIME, A.UNITS,REFERENCE_RESULT,B.TEST_CAUSE
                          FROM WIS_LAB_TEST_DETAIL A,WIS_LAB_TEST_MASTER B WHERE A.TEST_NO = B.TEST_NO AND B.PAT_ID = '{0}' 
                          AND VISIT_ID = {1} AND A.REPORT_ITEM_NAME= '{2}'  ORDER BY A.Result_date_time ",
                         ExtendApplicationContext.Current.PatientContext.PatientID, ExtendApplicationContext.Current.PatientContext.VisitID, _reportItemName);
