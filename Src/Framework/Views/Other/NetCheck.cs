    ...
    public partial class NetCheck : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void timerCheck_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    listBoxMsg.Items.Add("系统初始化尚未完成，麻醉系统将会自动关闭。");
                    listBoxMsg.Items.Add("请重新打开系统。");
                }
        ...
        private void NetCheck_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (ExtendApplicationContext.Current.SystemCurrentProcess < ProgramProcess.SystemAfterLoad)
            {
                ...
                //{
                //    DialogResult dialogResult = XtraMessageBox.Show("网络连接已恢复，是否重启麻醉系统？", "提示信息", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            else
            {
                if (NetworkInterruption)
                {
                    DialogResult dialogResult = XtraMessageBox.Show("网络连接中断，系统处于保护状态，无法操作，请稍等。是否强制退出麻醉系统？", "提示信息", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Application.DoEvents();
