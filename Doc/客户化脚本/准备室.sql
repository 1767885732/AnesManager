create or replace view prepare_PROCESS as
  select r.bed_label as pacu_bed_no, m.in_date_time as in_pacu_date_time, m.sequence, oldr.bed_label, i.inp_no, i.name, i.sex,
 to_char(floor((sysdate-m.in_date_time)*24), '09') || ':' || to_char(floor(mod((sysdate-m.in_date_time)*24,1)*60), '09')  as time_span,
 ...
 nvl(c1.user_name, m.pacu_doctor) as pacu_doctor, nvl(c2.user_name, m.reserved1) as pacu_nurse,r.location
...
create or replace view prepare_aquair as
select  * from (
select row_number() over(order by m.reserved9)  as AUQAIR_SEQUENCE,
    m.INQUIRY_BEFORE_DATE as AQUAIR_PACU_TIME, (case AT when 6 then '¡Ì' when 4 then '¡Á'  when 7 then '£½' else '' end) AQUAIRED,  m.INQUIRY_AFTER_DATE as AQUAIRED_TIME,
    ...
    m.patient_id, m.oper_id, m.visit_id, m.in_pacu_date_time, m.operating_room_no,(case when  m.out_date_time is null then '·ñ' else 'ÊÇ' end) IS_OUT,
    row_number() over(partition by AT order by m.INQUIRY_BEFORE_DATE) as DEAL_SEQUENCE, m.reserved6 Condition, pl.asa_grade, r.location
...
where (AT = 5 or AT = 6 or AT = 7) and  to_char(m.scheduled_date_time, 'yyyy-MM-dd') = to_char(sysdate, 'yyyy-MM-dd')
...
create or replace view prepare_aquair_all as
select * from (
select row_number() over(order by m.INQUIRY_BEFORE_DATE)  as AUQAIR_SEQUENCE,
    m.INQUIRY_BEFORE_DATE as AQUAIR_PACU_TIME, (case AT when 6 then '¡Ì' when 4 then '¡Á' else '' end) AQUAIRED,  m.INQUIRY_AFTER_DATE as AQUAIRED_TIME,
    r.bed_label, i.inp_no, i.name as pat_name, i.sex, extract(year from sysdate) - extract(year from i.date_of_birth) as age,
    nvl(ward.dept_name, h.ward_code) as Ward_Name, h.bed_no, m.operation_name,
    m.RESERVED7 as Out_place, nvl(su.user_name, m.surgeon) as Surgeon, m.anesthesia_method, m.oper_status,
    trim(nvl(fa.user_name, m.first_assistant) || ' ' || nvl(sa.user_name, m.second_assistant) || ' ' || nvl(ta.user_name, m.third_assistant)) as Assisatant,
    ...
    trim(nvl(sn1.user_name, m.first_supply_nurse) || ' ' || nvl(sn2.user_name, m.second_supply_nurse) ) as supply_nurse,
    m.patient_id, m.oper_id, m.visit_id, m.in_pacu_date_time, m.operating_room_no,(case when  m.out_date_time is null then '·ñ' else 'ÊÇ' end) IS_OUT,
     row_number() over(partition by AT order by m.INQUIRY_BEFORE_DATE) as DEAL_SEQUENCE, r.location
from med_operation_master m
left join med_operating_room r on m.operating_room = r.dept_code and m.operating_room_no = r.room_no and r.bed_type = 0
left join med_pat_master_index i on m.patient_id = i.patient_id
left join med_pats_in_hospital h on h.patient_id = m.patient_id and h.visit_id = m.visit_id
...
left join med_his_users sn1 on sn1.user_id = m.first_supply_nurse
left join med_his_users sn2 on sn2.user_id = m.second_supply_nurse
where (AT =4 or AT = 5 or AT = 6 or AT = 7) and  to_char(m.in_date_time, 'yyyy-MM-dd') = to_char(sysdate, 'yyyy-MM-dd')
order by AQUAIR_PACU_TIME)
where in_pacu_date_time is null and AQUAIR_PACU_TIME is not null
;
